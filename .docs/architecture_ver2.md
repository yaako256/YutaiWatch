...
- サイトにitem_keyがないため、内容ベースで自分で作ることにする。urlが擬似IDであるため、それを使って頑張る。構造体のitem_keyはなくした。
- init、monitorのほかにもう一つ、jsonlの古い方を削除するようなフローを作る(prune)
- pythonの方でスクレイピイング→構造体化まですることにした。jsonで受け取り、それをデシリアライズして色々する。
- loggerを別クレートで宣言。globalで持つような設計にした。
- errorログの送信ではなく、logsの送信にした。そのため、設定名や振る舞いが少し変わった。
- configを開発用と本番用で分けた。また、volume化した
- ditect_historyの更新有無はboolではなくusizeで個数を持つことににした。
